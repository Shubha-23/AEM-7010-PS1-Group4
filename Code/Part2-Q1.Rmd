...
```{r}
# 3rd Row
# replace na with 0 to create unbalanced sample of stores
ck1994_new$FTE_na0 <- ck1994_new$FTE
ck1994_new$FTE2_na0 <- ck1994_new$FTE2
col <- c("FTE","FTE2")
ck1994_new <- ck1994_new %>% mutate(FTE_na0 = ifelse(is.na(FTE_na0), 0, FTE_na0),
                                    FTE2_na0 = ifelse(is.na(FTE2_na0), 0, FTE2_na0))
ck1994_new$FTE_change_na0 <- ck1994_new$FTE2_na0 - ck1994_new$FTE_na0

# 2ed row
  T3R3 <- ck1994_new %>% 
          select(STATE, FTE_change_na0) %>%
          group_by(STATE) %>% # group_by state
          summarise(mean = mean(FTE_change_na0, na.rm = TRUE), 
                    var= var(FTE_change_na0, na.rm = TRUE), 
                    nasum = sum(is.na(FTE_change_na0)), 
                    N=n()) %>% # aggregate/summarize data
          mutate(n = N - nasum) %>% 
          mutate(se = sqrt(var/n))

# calculate differences
  T3R3_res <- bind_rows(T3R3, T3R3[2,]-T3R3[1,])
  T3R3_res$group<- c("PA", "NJ", "Difference")
  T3R3_res <- T3R3_res[,-1]
# Calculate SE for difference: SE = SQR(VAR/N + VAR/N)
  diff_se3 <- sqrt(T3R3_res$var[1]/T3R3_res$n[1] + T3R3_res$var[2]/T3R3_res$n[2])
  diff_se3
# Edit original cell
  T3R3_res$se[3] <- diff_se3
  
  kable(T3R3_res, digits=2)
